...
def main():
    ...
    logger.info('Move Index To Gpus')
    retriever.to_gpu()

    if os.path.isdir(args.query_reps):
        q_reps = []
        q_lookup = []
        dir_ents = os.listdir(args.query_reps)
        for de in dir_ents:
            tmp_reps, tmp_lookup = torch.load(os.path.join(args.query_reps, de))
            q_reps.append(tmp_reps)
            q_lookup.extend(tmp_lookup)
        q_reps = torch.cat(q_reps)
    else:
