...
from transformers import RobertaModel, RobertaPreTrainedModel, RobertaForSequenceClassification
from transformers.modeling_outputs import SequenceClassifierOutput, BaseModelOutput, Seq2SeqLMOutput

from .modeling_prefix_encoder import PrefixEncoder, PromptLstmEncoder
from .modeling_deberta import DebertaModel, DebertaPreTrainedModel, ContextPooler, StableDropout
...
class BertPromptForSequenceClassification(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.prefix_tokens = torch.arange(self.pre_seq_len).long()
        if config.prompt_from_vocab:
            raw_embedding = self.bert.get_input_embeddings()
            soft_embeds = raw_embedding.weight[:self.pre_seq_len].clone().detach()
            prompt_embeddings = torch.nn.Embedding.from_pretrained(soft_embeds, freeze=False)
        else:
            prompt_embeddings = torch.nn.Embedding(self.pre_seq_len, config.hidden_size)

        if config.prompt_encoder_type == "lstm":
            lstm_head = PromptLstmEncoder(config)
            self.prefix_encoder = torch.nn.Sequential(prompt_embeddings, lstm_head)
            ...
            self.prefix_encoder = torch.nn.Sequential(prompt_embeddings, mlp_head)
        else:
            self.prefix_encoder = torch.nn.Sequential(prompt_embeddings)
    ...
    def forward(
        ...
        for hs in outputs.hidden_states:
            hs = hs[:, self.pre_seq_len:, :].contiguous()
            outputs_hidden_states.append(hs)
...
class RobertaPromptForSequenceClassification(RobertaPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.prefix_tokens = torch.arange(self.pre_seq_len).long()
        if config.prompt_from_vocab:
            raw_embedding = self.roberta.get_input_embeddings()
            soft_embeds = raw_embedding.weight[:self.pre_seq_len].clone().detach()
            prompt_embeddings = torch.nn.Embedding.from_pretrained(soft_embeds, freeze=False)
        else:
            prompt_embeddings = torch.nn.Embedding(self.pre_seq_len, config.hidden_size)
